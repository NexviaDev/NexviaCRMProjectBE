...
`;
    ...
    async generateScheduleAnalysis(schedules) {
        ...
        const prompt = `
당신은 부동산 전문가이자 개인 코치입니다. 사용자의 일정을 분석하여 매우 구체적이고 개인적인 업무 관리 조언을 600제 이내로 제공하세요.
...
한국어로 전문적이고 실용적인 조언을 600자 이내로 제공해주세요.
`;
    ...
    async generateWeeklyBriefing(schedules, userName) {
        ...
        const prompt = `
당신은 부동산 CRM의 전략 코치입니다. ${userName}님의 이번 주 일정을 바탕으로 "언제, 어디서, 누구를" 만나는지 시간순으로 자연스럽게 정리하고, 형식에 얽매이지 말고 Gemini 스스로의 판단으로 가장 효과적인 접근 전략을 제안하세요. 지나치게 목차화하지 말고 흐름 있는 서술형 요약과 실전 조언을 섞어 주세요. (간결하게 하되 최대 길이: 1000자 이내 ) 
